...
//       fldOffset = fromTuple.copyFldOffset(); 
     ...
     *
     * @param fldNo the field number
     * @return the intervalType if success
     * @throws IOException                    I/O errors
     * @throws FieldNumberOutOfBoundException Tuple field number out of bound
     * @throws ClassNotFoundException         Error in object conversion
     */
    ...
    public IntervalType getIntervalFld(int fldNo)
        ...
        IntervalType val;
        if ((fldNo > 0) && (fldNo <= fldCnt)) {
            val = Convert.getIntervalTypeValue(fldOffset[fldNo - 1], data);
            return val;
        } else
            throw new FieldNumberOutOfBoundException(null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
    ...
    public Tuple setIntervalFld(int fldNo, IntervalType val)
            throws IOException, FieldNumberOutOfBoundException {
        if ((fldNo > 0) && (fldNo <= fldCnt)) {
            Convert.setIntervalTypeValue(val, fldOffset[fldNo - 1], data);
            return this;
        } else
            throw new FieldNumberOutOfBoundException(null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
    ...
    public void print(AttrType type[])
        ...
        float fval;
        String sval;
        IntervalType itVal = new IntervalType();
        ...
        for (i = 0; i < fldCnt - 1; i++) {
            switch (type[i].attrType) {
                ...
                case AttrType.attrInterval:
                    itVal.setStart(Convert.getIntValue(fldOffset[i], data));
                    itVal.setEnd(Convert.getIntValue(fldOffset[i + 4], data));
        ...
        switch (type[fldCnt - 1].attrType) {
            ...
            case AttrType.attrString:
                ...
                break;
            case AttrType.attrInterval:
                itVal = Convert.getIntervalTypeValue(fldOffset[i], data);
                System.out.println(itVal.toString());
    ...
    public void printTree(AttrType type[], int n1fields, int n2fields)
            throws IOException {
        System.out.println("Tree_Parent : [-1,-1]");
        IntervalType itVal = new IntervalType();
        String sval = Convert.getStrValue(fldOffset[0], data, fldOffset[1] - fldOffset[0]);
        System.out.print("\t");
        System.out.print(sval + ": ");
        itVal = Convert.getIntervalTypeValue(fldOffset[1], data);
        System.out.println(itVal.toString());

        int i, j, p, tab, comp_res = -10;
        int n1 = n1fields;
        int n2 = n2fields;

        Boolean match;

        for (i = 4; i < n1 - 1; i = i + 3) {
            tab = (i / 3) + 2;
            j = i - 3;
            match = false;
            while (j > 0 && !match) {
                try {
                    comp_res = TupleUtils.CompareTupleWithTuple(type[fldCnt + 1], this, j + 1, this, i + 1);
            ...
            if (match) {
                ...
                sval = Convert.getStrValue(fldOffset[i - 1], data, fldOffset[i] - fldOffset[i - 1]);
                itVal = Convert.getIntervalTypeValue(fldOffset[i], data);
                System.out.print(sval + ": ");
                System.out.println(itVal.toString());
            } else {
        ...
        sval = Convert.getStrValue(fldOffset[n1], data, fldOffset[n1 + 1] - fldOffset[n1]);
        System.out.print("\t");
        System.out.print(sval + ": ");
        itVal = Convert.getIntervalTypeValue(fldOffset[n1 + 1], data);
        System.out.println(itVal.toString());

        for (i = n1 + 4; i < (n1 + n2 - 1); i = i + 3) {
            tab = ((i - n1) / 3) + 2;
            j = i - 3;
            match = false;
            while (j > n1 && !match) {
    ...
    public void printmultinode(AttrType type[], int n1fields, int numTuples)
            throws IOException {
        System.out.println();
        System.out.println("Tree_Parent : [-1,-1]");
        IntervalType itVal = new IntervalType();

        int i, j, k, p, tab, comp_res = -10;
        int n1 = n1fields;

        Boolean match;

        for (k = 0; k < numTuples; k++) {
            String sval = Convert.getStrValue(fldOffset[k * n1], data, fldOffset[(k * n1) + 1] - fldOffset[k * n1]);
            System.out.print("\t");
            System.out.print(sval + ": ");
            itVal = Convert.getIntervalTypeValue(fldOffset[(k * n1) + 1], data);
            System.out.println(itVal.toString());
            for (i = (4 + k * n1); i < (n1 * (k + 1) - 1); i = i + 3) {
                tab = ((i - (k * n1)) / 3) + 2;
                j = i - 3;
                match = false;
                while (j > 0 && !match) {
