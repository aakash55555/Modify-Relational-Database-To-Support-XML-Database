...
public class TupleUtils {
        ...
        switch (fldType.attrType) {
            ...
            case AttrType.attrInterval:               //Compare two Intervals
                ...
                if (t1_it.getStart() == t2_it.getStart() && t1_it.getEnd() == t2_it.getEnd()) {
                    return 0;
                } else if (t1_it.getStart() < t2_it.getStart()) {
                    //T1 is Ancestor of T2
                    if (t1_it.getEnd() > t2_it.getEnd()) {
                        return -1;
                    } else {
                        return -2;
                    }
                } else if (t1_it.getStart() > t2_it.getStart()) {
                    //T1 is Descendent to T2
                    if (t1_it.getEnd() < t2_it.getEnd()) {
                        return 1;
                    } else {
                        return 2;
    ...
    public static boolean Equal(Tuple t1, Tuple t2, AttrType types[], int len)
            throws IOException, UnknowAttrType, TupleUtilsException, FieldNumberOutOfBoundException, ClassNotFoundException {
        int i;
    ...
    public static void SetValue(Tuple value, Tuple tuple, int fld_no, AttrType fldType)
        ...
        switch (fldType.attrType) {
            ...
            case AttrType.attrInterval:
                ...
                } catch (FieldNumberOutOfBoundException | ClassNotFoundException e) {
                    throw new TupleUtilsException(e, "FieldNumberOutOfBoundException is caught by TupleUtils.java");
                }
                break;
            default:
