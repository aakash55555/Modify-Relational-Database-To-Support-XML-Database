...
public class intervalTreeFile extends IndexFile implements GlobalConst {
    ...
    public intervalTreeFile(String filename)
        ...
        /*
         *
         * - headerPageId is the PageId of this intervalTreeFile's header page;
...
//    intervalTreeFile(java.lang.String filename, int delete_fashion) {
//    }
    ...
    /*
     *  Status intervalTreeFile::NaiveDelete (const void *key, const RID rid)
     *
    ...
    private boolean NaiveDelete(KeyClass key, RID rid)
        ...
        while (true) {
            ...
            while (entry == null) { // have to go right
                ...
                leafPage = new ITLeafPage(pinPage(nextpage),
                        headerPage.get_keyType());
                entry = leafPage.getFirst(new RID());
            }

            if (IT.keyCompare(key, entry.key) > 0)
                break;
    ...
    /*
     * findRunStart.
     * Status intervalTreeFile::findRunStart (const void   lo_key,
     *                                RID          *pstartrid)
     *
     ...
     *@return return a ITLeafPage instance which is pinned.
     ...
     * @throws ConstructPageException error in IT page constructor
    ...
    ITLeafPage findRunStart(KeyClass lo_key,
        ...
        if (pageno.pid == INVALID_PAGE) {        // no pages in the intervalTree
        ...
        while (IT.keyCompare(curEntry.key, lo_key) < 0) {
    ...
    private KeyClass _Delete(KeyClass key,
        ...
        if (sortPage.getType() == NodeType.LEAF) {
            ...
            while ((tmpEntry != null) && (IT.keyCompare(key, tmpEntry.key) >= 0)) {
                // WriteUpdateLog is done in the ITleafpage level - to log the
                // deletion of the rid.
                ...
                if (leafPage.delEntry(new KeyDataEntry(key, rid))) {
                    ...
                    if ((4 + leafPage.available_space()) <=
                        ...
                        // get a sibling
                        ITIndexPage parentPage;
                        parentPage =
                                new ITIndexPage(pinPage(parentPageId),
                                        headerPage.get_keyType());
        ...
        if (sortPage.getType() == NodeType.INDEX) {
            PageId childPageId;
            ITIndexPage indexPage = new ITIndexPage(page, headerPage.get_keyType());
            ...
            indexPage = new ITIndexPage(pinPage(currentPageId), headerPage.get_keyType());
            ...
            if ((4 /*sizeof slot*/ + indexPage.available_space()) <=
                ...
                // get a sibling
                ITIndexPage parentPage;
                parentPage = new ITIndexPage(pinPage(parentPageId),
                        headerPage.get_keyType());

                PageId siblingPageId = new PageId();
                ITIndexPage siblingPage;
                int direction;
                ...
                siblingPage = new ITIndexPage(pinPage(siblingPageId),
                        headerPage.get_keyType());

                int pushKeySize = 0;
                if (direction == 1) {
                    pushKeySize = IT.getKeyLength
                ...
                if (siblingPage.redistribute(indexPage, parentPage,
                    ...
                    // merge the two children
                    ITIndexPage leftChild, rightChild;
                    if (direction == -1) {
                        leftChild = siblingPage;
    ...
    void trace_children(PageId id)
        ...
        if (trace != null) {
            ...
            if (sortedPage.getType() == NodeType.INDEX) {
                ITIndexPage indexPage = new ITIndexPage(sortedPage, headerPage.get_keyType());
    ...
    private KeyDataEntry _insert(KeyClass key, RID rid,
        ...
        if (currentPage.getType() == NodeType.INDEX) {
            ITIndexPage currentIndexPage = new ITIndexPage(page,
                    headerPage.get_keyType());
            ...
            currentIndexPage = new ITIndexPage(pinPage(currentPageId),
                    headerPage.get_keyType());
            ...
            if (currentIndexPage.available_space() >=
                    IT.getKeyDataLength(upEntry.key, NodeType.INDEX)) {
            ...
            if (IT.keyCompare(upEntry.key, tmpEntry.key) >= 0) {
                ...
                tmpEntry =
                        IT.getEntryFromBytes(currentIndexPage.getpage(),
            ...
            if (currentLeafPage.available_space() >=
                    IT.getKeyDataLength(key, NodeType.LEAF)) {
                // no split has occurred
            ...
            if (IT.keyCompare(key, undoEntry.key) < 0) {
                //undo the final record
                if (currentLeafPage.available_space() <
                        newLeafPage.available_space()) {
    ...
    public void insert(KeyClass key, RID rid)
        ...
        if (IT.getKeyLength(key) > headerPage.get_maxKeySize())
        ...
        if (newRootEntry != null) {
            ...
            newRootPage = new ITIndexPage(headerPage.get_keyType());
            newRootPageId = newRootPage.getCurPage();
