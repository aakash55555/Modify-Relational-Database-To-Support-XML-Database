...
public class ITIndexPage extends ITSortedPage {
    /**
     * pin the page with pageno, and get the corresponding ITIndexPage,
     * also it sets the type of node to be NodeType.INDEX.
     *
     * @param pageno  Input parameter. To specify which page number the
     *                ITIndexPage will correspond to.
     * @param keyType either AttrType.attrInteger or AttrType.attrString.
     *                Input parameter.
     * @throws IOException            error from the lower layer
     * @throws ConstructPageException error when ITIndexPage constructor
     */
    public ITIndexPage(PageId pageno, int keyType)
            throws IOException,
    ...
    /**
     * associate the ITIndexPage instance with the Page instance,
     * also it sets the type of node to be NodeType.INDEX.
     *
     ...
     * @throws ConstructPageException error when ITIndexPage constructor
     */
    public ITIndexPage(Page page, int keyType)
            throws IOException,
            ConstructPageException {
        super(page, keyType);
        setType(NodeType.INDEX);
    ...
    /* new a page, associate the ITIndexPage instance with the Page instance,
     * also it sets the type of node to be NodeType.INDEX.
     *@param keyType either AttrType.attrInteger or AttrType.attrString.
     *  Input parameter.
     *@exception IOException  error from the lower layer
     *@exception ConstructPageException error when ITIndexPage constructor
     */
    public ITIndexPage(int keyType)
            throws IOException,
            ConstructPageException {
        super(keyType);
        setType(NodeType.INDEX);
    ...
    RID deleteKey(KeyClass key)
        ...
        try {
            ...
            while (IT.keyCompare(key, entry.key) > 0) {
                entry = getNext(rid);
                if (entry == null)
                    break;
            }
            ...
            else if (IT.keyCompare(key, entry.key) != 0)
     ...
     * ITIndexPage by intervalTree+ search algorithm
    ...
    PageId getPageNoByKey(KeyClass key)
        ...
        try {
            ...
            for (i = getSlotCnt() - 1; i >= 0; i--) {
                entry = IT.getEntryFromBytes(getpage(), getSlotOffset(i),
     ...
     * which  provide an iterator interface to the records on a ITIndexPage.
     *
    ...
    public KeyDataEntry getFirst(RID rid)
        ...
        try {
            ...
            entry = IT.getEntryFromBytes(getpage(), getSlotOffset(0),
                    getSlotLength(0),
     ...
     * iterator interface to the records on a ITIndexPage.
     *
    ...
    int getSibling(KeyClass key, PageId pageNo)
        ...
        try {
            ...
            for (i = getSlotCnt() - 1; i >= 0; i--) {
                ...
                if (IT.keyCompare(key, entry.key) >= 0) {
                    if (i != 0) {
                        entry = IT.getEntryFromBytes(getpage(), getSlotOffset(i - 1),
    ...
    /* find entry for key by intervalTree tree algorithm,
    ...
    /* find a key  by intervalTree+ algorithm,
     ...
     */
    boolean redistribute(ITIndexPage indexPage, ITIndexPage parentIndexPage,
        ...
        try {
            ...
            if (direction == -1) { // 'this' is the left sibling of indexPage
                if ((getSlotLength(getSlotCnt() - 1) + available_space()) >
                    ...
                    lastEntry = IT.getEntryFromBytes(getpage(), getSlotOffset(getSlotCnt() - 1),
                ...
                if ((getSlotLength(0) + available_space()) > ((MAX_SPACE - DPFIXED) / 2)) {
                    ...
                    firstEntry = IT.getEntryFromBytes(getpage(),
                            getSlotOffset(0),
                            getSlotLength(0), keyType, NodeType.INDEX);
