...
public class ITSortedPage extends HFPage {

    int keyType; //it will be initialized in ITFile
     ...
     *                ITSortedPage will correspond to.
     * @param keyType input parameter. It specifies the type of key. It can be
     *                AttrType.attrString or AttrType.attrInteger.
     * @throws ConstructPageException error for ITSortedPage constructor
     */
    public ITSortedPage(PageId pageno, int keyType)
     ...
     *                AttrType.attrString or AttrType.attrInteger.
     */
    public ITSortedPage(Page page, int keyType) {
     ...
     * @throws ConstructPageException error for ITSortedPage constructor
     */
    public ITSortedPage(int keyType)
            throws ConstructPageException {
        super();
        try {
            Page apage = new Page();
    ...
    protected RID insertRecord(KeyDataEntry entry)
        ...
        try {
            ...
            record = IT.getBytesFromEntry(entry);
            ...
            for (i = getSlotCnt() - 1; i > 0; i--) {
                ...
                key_i = IT.getEntryFromBytes(getpage(), getSlotOffset(i),
                        getSlotLength(i), keyType, nType).key;

                key_iplus1 = IT.getEntryFromBytes(getpage(), getSlotOffset(i - 1),
                        getSlotLength(i - 1), keyType, nType).key;

                if (IT.keyCompare(key_i, key_iplus1) < 0) {
                    // switch slots:
                    int ln, off;
                    ln = getSlotLength(i);
                    off = getSlotOffset(i);
